...
export default function CampaignDetailPage() {
  ...
  // Strip Google Maps tracking params from URLs
  const cleanUrl = (url: string): string => {
    try {
      const u = new URL(url);
      if (u.searchParams.get("utm_source") === "G_Business" || u.searchParams.has("utm_source")) {
        return u.origin; // just domain, no tracking junk
      }
      return url;
    } catch { return url; }
  };
  ...
  const handleSubmitForm = async (target: TargetRow) => {
    ...
    // senderEmail not required for preview — only warn if also missing at send time
    const rawUrl = target.contact_url || target.website || "";
    if (!rawUrl) { setToast("⚠️ ウェブサイトURLが見つかりません"); setTimeout(() => setToast(""), 3000); return; }
    const websiteUrl = cleanUrl(rawUrl);
    ...
    try {
      ...
      const data = await res.json();
      console.log("[form] preview response:", data);
      console.log("[form] generated message:", data.generatedMessage);
      const generatedMsg = data.generatedMessage || data.message || "";
      setFormModalMsg(generatedMsg);
      setFormModal({ targetId: target.id, websiteUrl, message: generatedMsg, senderName, senderEmail });
    } catch (err) { console.error("[form] preview error:", err); setToast("❌ メッセージ生成に失敗しました"); }
                ...
                {visibleTargets.map((t) => {
                  ...
                  return (
                    <div key={t.id} className="target-card" style={{
                      ...
                      {/* Action row */}
                      <div className="tc-row" style={{ display: "flex", alignItems: "center", gap: "8px", minWidth: 0, flexWrap: "wrap", rowGap: "6px" }}>
                      ...
                      {t.comment ? (
                        <div style={{ marginTop: "8px", background: "rgba(255,214,10,0.03)", border: "1px solid rgba(255,214,10,0.08)", borderRadius: "8px", padding: "10px 12px", overflow: "hidden", minWidth: 0 }}>
                          ...
                          <div className="tc-pre" style={{ fontSize: "14px", color: "rgba(240,239,232,0.75)", lineHeight: 1.8, minWidth: 0, overflowWrap: "break-word", wordBreak: "break-word" }}>
                           ...
                           const savedName = typeof window !== "undefined" ? localStorage.getItem("spark_sender_name") || "" : "";
                           const savedCompany = typeof window !== "undefined" ? localStorage.getItem("spark_company_name") || "" : "";
                           const senderLine = savedName
...
現在、βテスターとしてご協力いただける企業様を募集しております。無料でご利用いただけますので、ぜひ一度 https://spark-ai.jp をご覧いただけますと幸いです。

ご多忙の折、誠に恐れ入りますが、ご検討いただけますと幸いです。
どうぞよろしくお願いいたします。`}
                               </div>
                             </div>
                           );
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 2000, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
          <div style={{ background: "#13132a", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "20px", padding: "28px", width: "100%", maxWidth: "540px" }}>
            ...
            <div style={{ marginBottom: "12px" }}>
              <div style={{ fontSize: "11px", color: "rgba(240,239,232,0.4)", marginBottom: "4px" }}>送信先サイト</div>
              <a href={formModal.websiteUrl} target="_blank" rel="noopener noreferrer" style={{ fontSize: "12px", color: "#4285f4", wordBreak: "break-all", textDecoration: "none" }}>{formModal.websiteUrl}</a>
              <div style={{ marginTop: "6px", fontSize: "10px", color: "rgba(240,239,232,0.35)", lineHeight: 1.5 }}>
                PlaywrightがこのURLのお問い合わせフォームを自動検出し、メッセージを入力して送信します
              </div>
            </div>
            <div style={{ marginBottom: "12px" }}>
